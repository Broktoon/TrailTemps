/**
 * generate-normals-cdt.js
 *
 * Generates daily "normals" (365-day averaged arrays) for Continental Divide Trail points.
 * - Reads trails/continental-divide-trail/data/points.json (spine + all alt points)
 ...
 * - Computes average for each MM-DD across 2018–2024 (skips Feb 29)
 * - Stores grid_elev (ft, from API elevation field) for elevation correction in app.js
 * - Writes trails/continental-divide-trail/data/historical_weather.json
 *   (wrapped in { meta, points })
 ...
 *
 * Uses Open-Meteo Professional API (customer-archive-api.open-meteo.com).
 * Throttle: 2-second delay between requests.
 * Estimated time for ~700 points: ~24 minutes.
 *
 * To test with a small batch, set MAX_POINTS to e.g. 5.
 */
...
// ── Config ─────────────────────────────────────────────────────────────────────
const DATA_DIR    = path.join('trails', 'continental-divide-trail', 'data');
...
const SLEEP_MS   = 2000;   // Open-Meteo Professional — do not reduce
const MAX_POINTS = null;   // set to a number (e.g. 5) for a test run
...
function computeNormals(daily) {
  ...
  for (const [mmdd, bucket] of acc.entries()) {
    ...
    for (const f of fields) {
      const b = bucket[f];
      out[f][idx] = b.n ? Math.round((b.sum / b.n) * 10) / 10 : null;
    }
  }
...
async function main() {
  ...
  console.log(`  Spine points:              ${targetPoints.filter(p => !p.alt_id).length}`);
  console.log(`  Alt points:                ${targetPoints.filter(p => p.alt_id).length}`);
  ...
  // Remove stale entries whose IDs are no longer in points.json
  const validIds = new Set(targetPoints.map(p => String(p.id)));
  ...
  console.log(`\nExisting normals records:    ${hist.points.length}`);
  console.log(`Missing normals to generate: ${missing.length}`);

  if (missing.length === 0) {
    console.log('\u2714 All points already have normals. Nothing to do.');
    return;
  }
  ...
  for (const p of toProcess) {
    ...
    const loc = p.alt_id
      ? `${p.alt_id} alt_mile=${p.alt_mile}`
      : `mile=${p.mile}  state=${p.state}`;

    console.log(`\n[${processed + 1}/${toProcess.length}] ${id}  ${loc}`);
    ...
    try {
      ...
      if (hiCount < 330) {
        console.warn(`  \u26a0 Sparse coverage (hi days=${hiCount}). Still writing.`);
      }

      // grid_elev from API (meters → feet), used for elevation correction in app.js
      const gridElevFt = data.elevation != null
      ...
      const entry = {
        ...
        rh_lo:     normals.rh_lo,
        ws:        normals.ws,
        grid_elev: gridElevFt,
      };

      // Include routing fields for app.js lookup
      if (p.alt_id) {
        entry.alt_id   = p.alt_id;
        entry.alt_mile = p.alt_mile;
      } else {
        entry.mile  = p.mile;
        entry.state = p.state;
      }

      hist.points.push(entry);

      hist.meta.source    = `Open-Meteo ${DATASET} ${START_DATE}..${END_DATE}`;
      hist.meta.dataset   = DATASET;
      hist.meta.range     = `${START_DATE}..${END_DATE}`;
      hist.meta.units     = 'temperature: fahrenheit, wind: mph';
      hist.meta.generated = new Date().toISOString();
      writeJson(HIST_PATH, hist);

      processed++;
      console.log(`  \u2714 Written (hi coverage: ${hiCount}/365, grid_elev: ${gridElevFt} ft)`);
