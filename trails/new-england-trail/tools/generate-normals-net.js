/**
 * generate-normals-net.js
 *
 * Generates daily "normals" (365-day averaged arrays) for New England Trail points.
 * - Reads trails/new-england-trail/data/points.json
 * - Selects only 5-mile interval points (reduces API calls; ~50 points total)
 ...
 * - Writes to trails/new-england-trail/data/historical_weather.json
 * - Resume-safe: writes after EACH point; re-run to continue where it stopped
 *
 * Run: node trails/new-england-trail/tools/generate-normals-net.js
 *
...
const DATA_DIR   = path.join("trails", "new-england-trail", "data");
...
// Set to a number (e.g. 10) to process only first N missing points — useful for testing.
...
// ─── 5-MILE INTERVAL SELECTION ─────────────────────────────────────────────
//
// Main spine: miles 1, 5, 10, 15, …, 205, 208 (start + every-5 + end)
// Spur:       spur_miles 0, 5, 10, 15, 20, 25, 28 (start + every-5 + end)

const TARGET_MAIN_MILES = new Set([1]);
for (let m = 5; m <= 205; m += 5) TARGET_MAIN_MILES.add(m);
TARGET_MAIN_MILES.add(208);

const TARGET_SPUR_MILES = new Set([0, 5, 10, 15, 20, 25, 28]);

function isTargetPoint(p) {
  if (p.spur) return TARGET_SPUR_MILES.has(p.spur_mile);
  return TARGET_MAIN_MILES.has(p.mile);
}
...
function computeNormals(daily) {
  // Accumulate by MM-DD, skipping Feb 29
  const acc = new Map();
...
async function main() {
  ...
  // Select only 5-mile interval points
  const targetPoints = allPoints.filter(isTargetPoint);
  console.log(`Points in points.json:          ${allPoints.length}`);
  console.log(`Target 5-mile interval points:  ${targetPoints.length}`);
  ...
  for (const p of toProcess) {
    ...
    const mileLabel = p.spur
      ? `spur_mile=${p.spur_mile}`
      : `mile=${p.mile}`;
    ...
    console.log(`\n[${processed + 1}/${toProcess.length}] ${id} (${mileLabel}) lat=${lat} lon=${lon}`);
    ...
    try {
      ...
      // Store using FT-compatible key names (hi_app, lo_app) so app.js reuse is easy
      hist.points.push({
        id,
        lat,
        lon,
